...
public class MultiPlayerServer extends JFrame implements Runnable, ActionListener {
	...
	String[] strArr = null; // client에 server에 세팅된 버튼의 name을 보내기 위한 변수
	...
	public void makeCenter(int row, int column) {
		...
		for (int i = 1; i < btnList.size(); i+=2) {
			// 기존과 다르게 setName에서 홀수는 -2를 붙여 client단에서 카드뒤집기가 원할하게 만듬
	...
	public void makeNorth(String user_name) {
		...
		JLabel lb_1p_name = new JLabel(String.format("1P 사용자명: %s", user_name));
		...
		lb_2p_name = new JLabel(String.format("2P 사용자명: %s", user_name));
		...
		try {
			while(true) {
				...
				case "turnover": // 상대에게 클릭하는 권한을 넘길때
					...
					if(status.equals("T")) {
						JOptionPane.showMessageDialog(null, String.format("STATUS :: %s님이 클릭방해 아이템을 사용하였습니다.", name));
					}
				...
				case "2p_score": // 2P의 score를 갱신할때
					int a = (int) ret[1];
					lb_2p_score.setText(String.format("2P 점수: %d", a));
					int b = (int) ret[2];
					lb_2p_combo.setText(String.format("2P 연속콤보: %d", b));
					break;
					
				case "2p_combo": // 2P의 연속콤보값을 갱신할때
					lb_2p_combo.setText(String.format("2P 연속콤보: %d", (int) ret[1]));
					break;
					
				case "2p_name": // 2P가 입력한 사용자명을 1P에서 세팅
					name_2p = (String) ret[1];
					lb_2p_name.setText("2P 사용자명: " + name_2p);
					break;
					
				case "get_end_point": // 1P의 end_point를 2P에 전송
					end_point = (int) ret[1];
...
//						System.exit(0); // main종료
				...
				case "shuffle": // 2p에서 상대방 카드섞기 시전시
				...
				case "item_used": // 2p가 아이템을 사용하였을때
					name = (String)ret[1];
			...
			if(this.isActive()) { 
				// 1P가 dispose상태가 아닐때 2P가 나갔다는 Dialog창을 출력
				JOptionPane.showMessageDialog(null, String.format("[%s]님이 나갔다요!!!", name_2p));
			}
			try {
				if(client != null) {
					client.close();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(turnup_cnt == 2) { // 1번카드
			...
			if(val1 != val2) { // 뒤집은 카드의 두 값이 같지 않을때
				...
				continuously = 0;
				continuously_cnt = 0;
				lb_1p_combo.setText(String.format("1P 연속콤보: %d", continuously_cnt));
				try {
			...
			if(val1 == val2) { // 뒤집은 카드의 두 값이 같을때
				if(obj1 != obj2) { // 한장의 카드를 연속 3번누르기 방지
					...
					lb_1p_score.setText(String.format("1P 점수: %d", total_score));
					lb_1p_combo.setText(String.format("1P 연속콤보: %d", continuously_cnt));
					try {
	...
	*/
		...
		public void actionPerformed(ActionEvent e) {
			Object[] data = null;
			 if(e.getSource() == btn_interrupt) { // 클릭방해
				...
				data = new Object[]{"item_used", name_1p, turn_cont + "턴 연속 플레이"};
				--item_use_cnt;
                lb_item_use.setText("남은 사용 횟수 :: " + item_use_cnt);
                btn_item.setEnabled(false);       //아이템 버튼 disable
				three_turn_status = "T";
				...
				data = new Object[]{"item_used", name_1p, show_all + "초 전체보기"};
				sec = show_all;
				--item_use_cnt;
                lb_item_use.setText("남은 사용 횟수 :: " + item_use_cnt);
                btn_item.setEnabled(false);       //아이템 버튼 disable
				...
				data = new Object[]{"item_used", name_1p, "상대방 카드 섞기"};
				try {
					--item_use_cnt;
	                lb_item_use.setText("남은 사용 횟수 :: " + item_use_cnt);
	                btn_item.setEnabled(false);       //아이템 버튼 disable
	...
	class WhenYouClosed extends WindowAdapter{
		...
		public void windowClosed(WindowEvent e) {
			// dispose로 닫히는 상태(게임종료후 종료메시지 팝업후 확인버튼 클릭시)
			dao.deleteGameServer(name_1p);
		}
	}
